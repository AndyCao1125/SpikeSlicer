...
class LBHinge(nn.Module):
    ...
    def forward(self, prediction, label, target_bb=None):
        ...
        if self.error_metric.reduction == 'none':
            prediction = negative_mask.unsqueeze(1) * F.relu(prediction) + positive_mask.unsqueeze(1) * prediction
            loss = self.error_metric(prediction, (positive_mask * label).unsqueeze(1))
        else:

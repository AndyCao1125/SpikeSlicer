...
class DiMPActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        # print(f'mean iou_pred: {iou_pred.mean().item()}')
        # sys.exit(0)
        # Classification losses for the different optimization iterations
        if self.objective['iou'].reduction == 'none':
            clf_losses_test = [self.objective['test_clf'](s, data['test_label'], data['test_anno']).mean((1, 2, 3)) for s in target_scores]
        else:
        ...
        if self.objective['iou'].reduction == 'none':
            loss_iou = self.loss_weight['iou'] * self.objective['iou'](iou_pred, data['proposal_iou'].unsqueeze(1)).mean((1, 2))
        else:
        ...
        # Log stats
        stats = {'Loss/total': loss.mean().item(),
                 'Loss/iou': loss_iou.mean().item(),
                 'Loss/target_clf': loss_target_classifier.mean().item()}
        if 'test_init_clf' in self.loss_weight.keys():
            stats['Loss/test_init_clf'] = loss_test_init_clf.mean().item()
        if 'test_iter_clf' in self.loss_weight.keys():
            stats['Loss/test_iter_clf'] = loss_test_iter_clf.mean().item()
        stats['ClfTrain/test_loss'] = clf_loss_test.mean().item()
        if len(clf_losses_test) > 0:
            stats['ClfTrain/test_init_loss'] = clf_losses_test[0].mean().item()
            if len(clf_losses_test) > 2:
                stats['ClfTrain/test_iter_loss'] = sum(clf_losses_test[1:-1]).mean().item() / (len(clf_losses_test) - 2)
        stats['IOU/iou'] = iou_pred.mean().item()
...
class TaMOsActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        if 'highres' in bbox_preds:
            ...
            stats['Loss/weighted_GIoU_highres'] = self.loss_weight['giou'] * loss_giou_highres.mean().item()
        ...
        stats.update({
            ...
            'Loss/GIoU': loss_giou.mean().item(),
            'Loss/weighted_GIoU': self.loss_weight['giou'] * loss_giou.mean().item(),
            'Loss/clf_loss_test': clf_loss_test.mean().item(),
...
class KLDiMPActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        if 'clf_ce' in self.loss_weight.keys():
            # Classification losses for the different optimization iterations
            if self.objective['bb_ce'].reduction == 'none':
                clf_ce_losses = [self.objective['clf_ce'](s, data['test_label_density'], grid_dim=(-2,-1), reduction='none') for s in target_scores]
            else:
                clf_ce_losses = [self.objective['clf_ce'](s, data['test_label_density'], grid_dim=(-2,-1), reduction='mean') for s in target_scores]
        ...
        # Log stats
        stats = {'Loss/total': loss.mean().item(),
                 'Loss/bb_ce': bb_ce.mean().item(),
                 'Loss/loss_bb_ce': loss_bb_ce.mean().item()}
        if 'test_clf' in self.loss_weight.keys():
            stats['Loss/target_clf'] = loss_target_classifier.mean().item()
        if 'test_init_clf' in self.loss_weight.keys():
            stats['Loss/test_init_clf'] = loss_test_init_clf.mean().item()
        if 'test_iter_clf' in self.loss_weight.keys():
            stats['Loss/test_iter_clf'] = loss_test_iter_clf.mean().item()
        if 'clf_ce' in self.loss_weight.keys():
            stats['Loss/clf_ce'] = loss_clf_ce.mean().item()
        if 'clf_ce_init' in self.loss_weight.keys():
            stats['Loss/clf_ce_init'] = loss_clf_ce_init.mean().item()
        if 'clf_ce_iter' in self.loss_weight.keys() and len(clf_ce_losses) > 2:
            stats['Loss/clf_ce_iter'] = loss_clf_ce_iter.mean().item()
        ...
        if 'clf_ce' in self.loss_weight.keys():
            stats['ClfTrain/clf_ce'] = clf_ce.mean().item()
            if len(clf_ce_losses) > 0:
                stats['ClfTrain/clf_ce_init'] = clf_ce_losses[0].mean().item()
                if len(clf_ce_losses) > 2:
                    stats['ClfTrain/clf_ce_iter'] = sum(clf_ce_losses[1:-1]).mean().item() / (len(clf_ce_losses) - 2)
        stats['IOU/bb_scores'] = bb_scores.mean().item()
...
class TranstActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        losses = torch.stack([loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict], dim=0).mean(0)

        # Return training stats
        stats = {'Loss/total': losses.mean().item(),
                 'Loss/ce': loss_dict['loss_ce'].mean().item(),
                 'Loss/bbox': loss_dict['loss_bbox'].mean().item(),
                 'Loss/giou': loss_dict['loss_giou'].mean().item(),
                 'iou': loss_dict['iou'].mean().item()
                 }

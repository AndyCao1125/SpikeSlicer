...
def percentile(t, q):
    ...
    result = t.view(B, C, -1).kthvalue(k).values
...
class EDiMPProcessing(BaseProcessing):
    ...
    #              mode='pair', train_mode='spikeslicer', *args, **kwargs):
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor, crop_type='replicate',
                 max_scale_change=None, mode='pair', proposal_params=None, label_function_params=None, train_mode='spikeslicer', *args, **kwargs):
        """
        args:
            search_area_factor - The size of the search region relative to the target size.
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['train', 'test']:
            ...
            # Add a uniform noise to the center pos
            if self.train_mode == 'spikeslicer':
            ...
            # Apply transforms
            if self.transform[s] is not None:
                data[s + '_images'], data[s + '_anno'] = self.transform[s](image=crops, bbox=boxes, joint=False)
            else:
                crop_frames = torch.cat([torch.from_numpy(crop.transpose(2, 0, 1)[np.newaxis, :]) for crop in crops]).to(torch.float32)
                B, C = crop_frames.shape[:2]
                percent = percentile(crop_frames, 99.99)
                for b in range(B):
                    for c in range(C):
                        crop_frames[b, c] = crop_frames[b, c].clamp(max=percent[b, c])
                data[s + '_images'] = crop_frames
                data[s + '_anno'] = torch.cat([box.unsqueeze(0) for box in boxes])
...
class TransTProcessing(TransTBaseProcessing):
    """ The processing class used for training TransT. The images are processed in the following way.
    First, the target bounding box is jittered by adding some noise. Next, a square region (called search region )
    centered at the jittered target center, and of area search_area_factor^2 times the area of the jittered box is
    cropped from the image. The reason for jittering the target box is to avoid learning the bias that the target is
    ...
    def __call__(self, data: TensorDict):
        ...
        # data['template_anno'] = data['template_anno']
        # data['search_anno'] = data['search_anno']
        return data
...
class EKLDiMPProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor, crop_type='replicate',
                 ...
                 label_function_params=None, label_density_params=None, train_mode='spikeslicer', *args, **kwargs):
        """
        args:
            search_area_factor - The size of the search region  relative to the target size.
            output_sz - An integer, denoting the size to which the search region is resized. The search region is always
...
class EKYSProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_param, scale_jitter_param,
                 proposal_params=None, label_function_params=None, min_crop_inside_ratio=0, train_mode='spikeslicer',
                 *args, **kwargs):
        """
        args:
            search_area_factor - The size of the search region  relative to the target size.
...
class TaMOsProcessing(BaseProcessing):
    ...
    def _generate_ltbr_regression_targets(self, target_bb, output_sz, stride, radius):
        ...
        if turn and stride == 8:
            hsz = output_sz[1] // stride - 1
        nb = target_bb.shape[0]

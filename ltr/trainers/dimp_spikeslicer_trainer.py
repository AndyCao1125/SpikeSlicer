...
class DIMP_SpikeSlicerTrainer(BaseTrainer):
    ...
    def cycle_dataset_snn(self, loader):
        ...
        if 'clf_ce' in self.actor.loss_weight.keys():
            self.actor.loss_weight['clf_ce'] = 0
        self.processing.train_mode = 'spikeslicer'
        if self.train_model_name == 'dimp50':
            self.actor.objective['iou'].reduction = 'none'
            self.actor.objective['test_clf'].error_metric.reduction = 'none'
        elif self.train_model_name == "prdimp50":
            self.actor.objective['bb_ce'].reduction = 'none'
        ...
        for i, data in enumerate(loader, 1):
            ...
            data['train_images'] = data['train_images'].unsqueeze(0).repeat(both_extend_steps, 1, 1, 1, 1).flatten(0, 1).permute(0, 2, 3, 1).contiguous().cpu().numpy()
            data['test_images'] = test_images.flatten(0, 1).permute(0, 2, 3, 1).contiguous().cpu().numpy()
            ...
            data['test_proposals'] = data['test_proposals'].unsqueeze(1).repeat(1, 3, 1, 1)
            if 'proposal_density' in data.keys():
                data['proposal_density'] = data['proposal_density'].unsqueeze(1).repeat(1, 3, 1)
            if 'gt_density' in data.keys():
                data['gt_density'] = data['gt_density'].unsqueeze(1).repeat(1, 3, 1)
    ...
    def cycle_dataset_ann(self, loader):
        ...
        self.processing.train_mode = 'dimp'
        if self.train_model_name == 'dimp50':
            self.actor.objective['iou'].reduction = 'mean'
            self.actor.objective['test_clf'].error_metric.reduction = 'mean'
        elif self.train_model_name == "prdimp50":
            self.actor.objective['bb_ce'].reduction = 'mean'
        self.actor.train(loader.training)
        torch.set_grad_enabled(loader.training)
        self._init_timing()
        functional.set_step_mode(self.snn, "m")
        for i, data in enumerate(loader, 1):
            ...
            if self.epoch >= self.split_train_epoch:
                ...
                start_idx_ls = torch.stack(start_idx_ls, dim=0).to(data['train_anno'].device)
                spike_local_idx_ls = torch.stack(spike_local_idx_ls, dim=0).to(data['train_anno'].device)
            ...
            for idx in range(3):
                ...
                spike_local_idx_ls.append(spike_local_idx)
            start_idx_ls = torch.stack(start_idx_ls, dim=0).to(data['test_anno'].device)
            spike_local_idx_ls = torch.stack(spike_local_idx_ls, dim=0).to(data['test_anno'].device)
            ...
            data['train_images'] = data['train_images'].permute(0, 1, 3, 4, 2).contiguous().flatten(0, 1).cpu().numpy()
            data['test_images'] = data['test_images'].permute(0, 1, 3, 4, 2).contiguous().flatten(0, 1).cpu().numpy()
            data['test_anno'] = data['test_anno'].flatten(0, 1).cpu()
            data['train_anno'] = data['train_anno'].flatten(0, 1).cpu()

            data = self.processing(data)
            # data['test_images'] = data['test_images'].view(*pre_shape, *data['test_images'].shape[1:])
            # data['train_images'] = data['train_images'].view(*pre_shape, *data['train_images'].shape[1:])
            # data['train_anno'] = data['train_anno'].view(anno_shape)
            # data['test_anno'] = data['test_anno'].view(anno_shape)

            # data['test_proposals'] = data['test_proposals'].view(*pre_shape, *data['test_proposals'].shape[1:])
            # if 'proposal_density' in data.keys():
            #     data['proposal_density'] = data['proposal_density'].view(*pre_shape, *data['proposal_density'].shape[1:])
            # if 'gt_density' in data.keys():
            #     data['gt_density'] = data['gt_density'].view(*pre_shape, *data['gt_density'].shape[1:])
            # for key in ['test_images', 'train_images', 'test_anno', 'train_anno', 'test_proposals', 'test_label', 'proposal_iou']:
            #     data[key] = data[key].to(self.device)
            for key in data.keys():
                if isinstance(data[key], torch.Tensor):

...
def calc_seq_err_robust_transt(pred_bb, anno_bb, dataset, target_visible=None):
    pred_bb = pred_bb.clone()
...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            print(f'Loading {results_path}, seqnameP: {seq.name}, results_dir: {trk.results_dir}')
...
def extract_results_fe108(trackers, dataset, report_name, eval_dir=None, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            trk.results_dir = eval_dir
            base_results_path = '{}/{}'.format(trk.results_dir, seq.name)
            results_path = '{}.txt'.format(base_results_path)

            results_frameidx_path = '{}_frameidx.txt'.format(base_results_path)

            # print(f'results_path:{results_path}, results_frameidx_path:{results_frameidx_path}')
            # exit()

            if os.path.isfile(results_path):
                pred_bb = torch.tensor(load_text(str(results_path), delimiter=('\t', ','), dtype=np.float64))
                expected_idx = torch.tensor(load_text(str(results_frameidx_path), delimiter=('\t', ','), dtype=np.float64))
            else:
                if skip_missing_seq:
                    valid_sequence[seq_id] = 0
                    break
            ...
            anno = []
            for idx in range(len(expected_idx)):
                if idx == 0:
                    anno_ = get_united_bbox(anno_bb, 0, expected_idx[idx].long())
                else:
                    anno_ = get_united_bbox(anno_bb, expected_idx[idx-1].long() + 1, expected_idx[idx].long())
                anno.append(anno_)
            anno_bb = torch.stack(anno, dim=0)
            # anno_bb = anno_bb[expected_idx.long()]

            # print(group1_bb[0:10])
            # print(anno_bb[0:10])
            # print(f'group1_bbshape:{group1_bb.shape}')
            # print(f'equal:{torch.equal(torch.round(group1_bb[:1],decimals=4), torch.round(anno_bb[:1],decimals=4))}')
            # # diff_positions = torch.nonzero(group1_bb != anno_bb)
            # # print(f'diff_positions:{diff_positions}')
            # # print(f'exp_idxshape:{expected_idx.shape}')
            # # print(f'anno_bbshape:{anno_bb.shape}')
            # # print(f'pred_bb_bbshape:{pred_bb.shape}')
            # sys.exit(0)
            # Calculate measures
            if trk.name == 'transt':
                err_overlap, err_center, err_center_normalized, valid_frame = calc_seq_err_robust_transt(
                pred_bb, anno_bb, seq.dataset, target_visible)
            else:
                err_overlap, err_center, err_center_normalized, valid_frame = calc_seq_err_robust(
                    pred_bb, anno_bb, seq.dataset, target_visible)

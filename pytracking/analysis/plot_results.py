...
from pytracking.analysis.extract_results import extract_results, extract_results_fe108
...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    tikzplotlib_fix_ncols(fig)  ## fix for matplotlib 3.6
    ...
    fig.savefig('{}/{}_plot.pdf'.format(result_plot_path, plot_type), dpi=300, format='pdf')
...
def extract_zip_if_needed(eval_dir):
    if eval_dir.endswith('.zip'):
        print('loading a zip file, unzip first')
        current_dir = os.path.dirname(eval_dir)
        folder_name = os.path.splitext(os.path.basename(eval_dir))[0]
        extracted_dir = os.path.join(current_dir, folder_name)
        
        with zipfile.ZipFile(eval_dir, 'r') as zip_ref:
             zip_ref.extractall(extracted_dir)
        
        eval_dir = extracted_dir
        print(f"unzip to:{eval_dir}")
    
    return eval_dir



def check_and_load_precomputed_results(trackers, dataset, report_name, force_evaluation=False, eval_dir=None, **kwargs):
    # Load data
    settings = env_settings()

    eval_data_path = os.path.join(eval_dir, 'eval_data.pkl')
    print('eval_data_path: ', eval_data_path)

    eval_dir = extract_zip_if_needed(eval_dir)

    eval_fe108 = True   ## this must be true for testing FE108 dataset

    print('eval_fe108: ', eval_fe108)

    if eval_fe108:
        if os.path.isfile(eval_data_path) and not force_evaluation:
            print('Pre-computed evaluation data found. Loading results!')
            with open(eval_data_path, 'rb') as fh:
                eval_data = pickle.load(fh)
        else:
            print('Pre-computed evaluation data not found. Computing results!')
            eval_data = extract_results_fe108(trackers, dataset, report_name, eval_dir,  **kwargs)

        if not check_eval_data_is_valid(eval_data, trackers, dataset):
            # print('Pre-computed evaluation data invalid. Re-computing results!')
            eval_data = extract_results_fe108(trackers, dataset, report_name, **kwargs)
        else:
            # Update display names
            tracker_names = [{'name': t.name, 'param': t.parameter_name, 'run_id': t.run_id, 'disp_name': t.display_name}
                            for t in trackers]
...
def plot_results(trackers, dataset, report_name, merge_results=False,
    ...
    if 'success' in plot_types:
        ...
        # print(f'ave_success_rate_plot_overlap: {ave_success_rate_plot_overlap}')
...
def print_results(trackers, dataset, report_name, merge_results=False,
                  plot_types=('success'), eval_dir=None, **kwargs):
    ...
    """
    # Load pre-computed results
    eval_data = check_and_load_precomputed_results(trackers, dataset, report_name, eval_dir=eval_dir, **kwargs)
